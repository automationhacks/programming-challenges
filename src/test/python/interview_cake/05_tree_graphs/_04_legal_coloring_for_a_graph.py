...
How can we be sure we'll always have at least one legal color for every node? In a graph with
maximum degree D, each node has at most D neighbors. That means there are at most D colors
taken by a node's neighbors. And we have D+1 colors, so there's always at least one color left
to use.

When we color each node, we're careful to stop iterating over colors as soon as we find a legal
color.
"""
...
def color_graph(graph, colors):
    for node in graph:
        if node in node.neighbors:
            raise Exception(f'Legal coloring is impossible for a node: {node.label} with loop')
        # Check all neighbors colors and add it to a set so that lookup can be
        # done in O(1) time
        illegal_colors = set(
            [neighbor.color for neighbor in node.neighbors if neighbor.color]
        )

        # Find the first legal color and assign it to the node
        for color in colors:
